...
$.widget("ui.board", {
           ...
           _createHexes: function() {
             var board = this;
             var hexesDiv = $("<div/>").addClass("hexes").appendTo(this.element);
             var hexesUl = $("<ul/>").appendTo(hexesDiv);
             for(var row = 0; row < this._getBoard().getHeight(); row++) {
               var rowLi = $("<li/>").addClass("row-" + row).appendTo(hexesUl);
               var rowUl = $("<ul/>").appendTo(rowLi);
               for(var col = 0; col < this._getBoard().getWidth(); col++) {
                 $("<li/>").addClass("col-" + col).appendTo(rowUl);
               }
             }
             $.each(this._getBoard().getHexes(),
                    function() {
                      board._createHex(this);
                    }
                   );
           },
           ...
           _createHex: function(hex) {
             var board = this;
             var hexLi = this.element.find(".hexes .row-" + hex.getRow() + " .col-" + hex.getCol()).addClass(hex.getType());
           ...
           _createNode: function(node) {
             var board = this;
             var nodeLi = this.element.find(".nodes .row-" + node.getRow() + " .col-" + node.getCol());
           ...
           _createEdge: function(edge) {
             var board = this;
             var edgeLi = this.element.find(".edges .row-" + edge.getRow() + " .col-" + edge.getCol());
